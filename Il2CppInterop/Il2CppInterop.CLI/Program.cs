...
var generateCommand = new Command("generate")
{
    ...
    new Option<bool>("--runtime-metadata-only",
        "Resolve proxy methods only from runtime IL2CPP metadata; ignore offline method addresses and tokens."),
    new Option<FileInfo>("--type-allowlist",
        "UTF-8 assembly|full-type-name allowlist used to generate a dependency-closed proxy subset.").ExistingOnly(),
...
internal record GenerateCommandOptions(
    ...
    bool NoXrefCache,
    bool RuntimeMetadataOnly,
    FileInfo? TypeAllowlist,
    string[]? AddPrefixTo,
...
) : BaseCmdOptions(Verbose)
{
    public override CmdOptionsResult Build()
    {
        ...
        opts.RuntimeMetadataOnly = RuntimeMetadataOnly;
        opts.GameAssemblyPath = RuntimeMetadataOnly ? "" : GameAssembly?.FullName ?? "";
        opts.NoXrefCache = NoXrefCache || RuntimeMetadataOnly;
        ...
        if (TypeAllowlist is not null)
            opts.ReadTypeAllowList(TypeAllowlist.FullName);

        if (RuntimeMetadataOnly && GameAssembly is not null)
        {
            res.Logger.LogWarning(
                "Ignoring --game-assembly because --runtime-metadata-only forbids offline method-address analysis.");
        }
