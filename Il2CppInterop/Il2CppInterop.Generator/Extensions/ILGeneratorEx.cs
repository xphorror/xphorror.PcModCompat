...
{
    private static string GetProxyTypeIdentity(TypeRewriteContext enclosingType)
        => $"{enclosingType.OriginalType.DeclaringModule?.Name}:{enclosingType.OriginalType.FullName}";

    private static void EmitGuardedObjectUnbox(
        this ILProcessor body,
        RuntimeAssemblyReferences imports,
        string identity)
    {
        body.Add(OpCodes.Ldstr, identity + " object");
        body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppObject.Value);
        body.Add(OpCodes.Call, imports.IL2CPP_il2cpp_object_unbox.Value);
        body.Add(OpCodes.Ldstr, identity + " unboxed data");
        body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppPointer.Value);
    }

    private static void EmitGuardedObjectClass(
        this ILProcessor body,
        RuntimeAssemblyReferences imports,
        string identity)
    {
        ...
        body.Add(OpCodes.Ldstr, identity + " class");
        body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppClass.Value);
    }
        ...
        TypeRewriteContext enclosingType, int argumentIndex)
    {
        ...
        else if (originalType.IsValueType())
        {
            ...
            else
            {
                ...
                body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " field store");
                ...
                body.Add(OpCodes.Ldstr, GetProxyTypeIdentity(enclosingType) + " field value class");
                body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppClass.Value);
        ...
        TypeRewriteContext enclosingType, int argumentIndex)
    {
        ...
        body.Add(OpCodes.Dup);
        body.EmitGuardedObjectClass(imports, GetProxyTypeIdentity(enclosingType) + " generic field store");
        ...
        body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " generic field store");
        body.Add(OpCodes.Ldloc, tempLocal);
        body.EmitGuardedObjectClass(imports, GetProxyTypeIdentity(enclosingType) + " generic field size");
        body.Add(OpCodes.Ldc_I4_0);
        body.Add(OpCodes.Conv_U);
        body.Add(OpCodes.Call, imports.IL2CPP_il2cpp_class_value_size.Value);
        body.Add(OpCodes.Cpblk);
        ...
        bool unboxNonBlittableType, bool unboxNonBlittableGeneric, out CilLocalVariable? refVariable)
    {
        ...
        if (originalType is ByReferenceTypeSignature)
        {
            ...
            else if (originalType.GetElementType().IsValueType())
            {
                ...
                if (unboxNonBlittableType)
                    body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " by-ref argument");
            }
            else
            {
        ...
        else if (originalType.IsValueType())
        {
            ...
            else
            {
                ...
                body.Add(OpCodes.Call, imports.IL2CPP_Il2CppObjectBaseToPtrNotNull.Value);
                if (unboxNonBlittableType)
                    body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " value argument");
            }
        }
        ...
        bool valueTypeArgument0IsAPointer, bool allowNullable, bool unboxNonBlittableType)
    {
        ...
        if (unboxNonBlittableType)
        {
            ...
            body.Add(OpCodes.Dup);
            body.EmitGuardedObjectClass(imports, GetProxyTypeIdentity(enclosingType) + " generic argument");
            body.Add(OpCodes.Call, imports.IL2CPP_il2cpp_class_is_valuetype.Value);
            body.Add(OpCodes.Brfalse_S, finalNop); // return reference types immediately
            body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " generic argument");
        }
        ...
        bool extraDerefForNonValueTypes, bool unboxValueType)
    {
        ...
        else if (originalReturnType.IsValueType())
        {
            if (convertedReturnType.IsValueType())
            {
                body.Add(OpCodes.Ldloc, pointerVariable);
                if (unboxValueType)
                    body.EmitGuardedObjectUnbox(imports, GetProxyTypeIdentity(enclosingType) + " return value");
            ...
            else
            {
                ...
                else
                {
                    ...
                    body.Add(OpCodes.Ldsfld, enclosingType.NewType.DeclaringModule!.DefaultImporter.ImportField(classPointerFieldRef));
                    body.Add(OpCodes.Ldstr, GetProxyTypeIdentity(enclosingType) + " boxed return class");
                    body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppClass.Value);
                    body.Add(OpCodes.Ldloc, pointerVariable);
                    body.Add(OpCodes.Call, imports.IL2CPP_il2cpp_value_box.Value);
                    body.Add(OpCodes.Ldstr, GetProxyTypeIdentity(enclosingType) + " boxed return object");
                    body.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppObject.Value);
                }
        ...
        IFieldDescriptor classHandle, TypeSignature il2CppObjectTypeDef)
    {
        ...
        methodBody.Add(OpCodes.Ldstr, targetType.FullName + " box class");
        methodBody.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppClass.Value);
        ...
        methodBody.Add(OpCodes.Ldstr, targetType.FullName + " boxed object");
        methodBody.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppObject.Value);
