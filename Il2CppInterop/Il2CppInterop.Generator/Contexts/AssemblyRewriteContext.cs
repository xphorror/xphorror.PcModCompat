...
public class AssemblyRewriteContext
{
    ...
    public TypeSignature RewriteTypeRef(TypeSignature? typeRef)
    {
        ...
        var originalTypeDef = typeRef.Resolve() ?? throw new InvalidOperationException(
            $"Could not resolve proxy signature type {typeRef.FullName} " +
            $"(scope={typeRef.Scope?.Name}, outputAssembly={NewAssembly.Name}).");
