...
#nullable enable
    ...
    public void AddMembers()
    {
        ...
        {
            if (!AssemblyContext.GlobalContext.Options.ShouldGenerateField(OriginalType, originalTypeField))
                continue;
            if (originalTypeField.Signature is null)
            {
                Logger.Instance.LogWarning(
                    "Skipping malformed field without a signature: {Assembly}::{Type}.{Field}",
                    OriginalType.DeclaringModule?.Name,
                    OriginalType.FullName,
                    originalTypeField.Name);
                continue;
            }
        ...
        {
            if (!AssemblyContext.GlobalContext.Options.ShouldGenerateMethod(OriginalType, originalTypeMethod))
                continue;
