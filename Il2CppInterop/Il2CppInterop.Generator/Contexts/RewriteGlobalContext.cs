...
public class RewriteGlobalContext : IDisposable
{
        ...
        IMetadataAccess unityAssemblies)
    {
        ...
        foreach (var sourceAssembly in gameAssemblies.Assemblies)
        {
            ...
            if (!options.ShouldGenerateAssembly(assemblyName))
                continue;
        ...
        if (!myAssemblies.ContainsKey("mscorlib"))
            throw new InvalidOperationException("Generator input/type allowlist must include mscorlib.");
    }
