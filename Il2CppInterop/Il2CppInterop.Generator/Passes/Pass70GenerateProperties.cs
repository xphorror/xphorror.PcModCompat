    ...
    public static void DoPass(RewriteGlobalContext context)
    {
        foreach (var assemblyContext in context.Assemblies)
            foreach (var typeContext in assemblyContext.Types)
            {
                ...
                {
                    if (!context.Options.ShouldGenerateProperty(type, oldProperty))
                        continue;
                    ...
                    property.SetSemanticMethods(
                        oldProperty.GetMethod is null ? null : typeContext.TryGetMethodByOldMethod(oldProperty.GetMethod)?.NewMethod,
                        oldProperty.SetMethod is null ? null : typeContext.TryGetMethodByOldMethod(oldProperty.SetMethod)?.NewMethod);
                }
