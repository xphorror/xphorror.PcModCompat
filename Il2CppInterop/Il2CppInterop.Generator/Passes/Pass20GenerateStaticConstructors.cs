...
public static class Pass20GenerateStaticConstructors
{
    ...
    private static void GenerateStaticProxy(AssemblyRewriteContext assemblyContext, TypeRewriteContext typeContext)
    {
        ...
        var typeIdentity = $"{oldType.DeclaringModule?.Name}:{oldType.FullName}";
        ctorBuilder.Add(OpCodes.Ldstr, typeIdentity);
        ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_RequireIl2CppClass.Value);
        ...
        if (oldType.HasGenericParameters())
        {
            ...
            ctorBuilder.Add(OpCodes.Ldstr, typeIdentity);
            ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_GetIl2CppTypeForClass.Value);
            ...
            for (var i = 0; i < oldType.GenericParameters.Count; i++)
            {
                ...
                var classPointerGetter = new MemberReference(
                    storeRef.ToTypeDefOrRef(),
                    "GetNativeClassPointerForGenericArgument",
                    MethodSignature.CreateStatic(assemblyContext.Imports.Module.IntPtr()));
                ctorBuilder.Add(OpCodes.Call, classPointerGetter);
                ctorBuilder.Add(OpCodes.Ldstr, $"{typeIdentity} generic argument {param.Name}");
                ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_GetIl2CppTypeForClass.Value);
            ...
            ctorBuilder.Add(OpCodes.Ldstr, typeIdentity + " inflated type handle");
            ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_RequireIl2CppPointer.Value);
            ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_il2cpp_class_from_type.Value);
            ctorBuilder.Add(OpCodes.Ldstr, typeIdentity + " inflated class");
            ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_RequireIl2CppClass.Value);
        }
        ...
        foreach (var method in typeContext.Methods)
        {
            ...
            if (assemblyContext.GlobalContext.Options.RuntimeMetadataOnly)
            {
                EmitExactMethodLookup(ctorBuilder, assemblyContext, method);
            }
            else if (token == 0)
            {
                ourTokenlessMethods++;
    ...
    private static void EmitExactMethodLookup(
        ILProcessor ctorBuilder,
        AssemblyRewriteContext assemblyContext,
        MethodRewriteContext method)
    {
        var originalMethod = method.OriginalMethod;
        ctorBuilder.Add(originalMethod.GenericParameters.Count > 0 ? OpCodes.Ldc_I4_1 : OpCodes.Ldc_I4_0);
        ctorBuilder.Add(originalMethod.IsStatic ? OpCodes.Ldc_I4_1 : OpCodes.Ldc_I4_0);
        ctorBuilder.Add(OpCodes.Ldc_I4, originalMethod.GenericParameters.Count);
        ctorBuilder.Add(OpCodes.Ldstr, originalMethod.Name!);
        ctorBuilder.EmitLoadTypeNameString(
            assemblyContext.Imports,
            originalMethod,
            originalMethod.Signature!.ReturnType,
            method.NewMethod.Signature!.ReturnType);
        ctorBuilder.Add(OpCodes.Ldc_I4, originalMethod.Parameters.Count);
        ctorBuilder.Add(OpCodes.Newarr, assemblyContext.Imports.Module.String().ToTypeDefOrRef());

        for (var index = 0; index < originalMethod.Parameters.Count; index++)
        {
            ctorBuilder.Add(OpCodes.Dup);
            ctorBuilder.Add(OpCodes.Ldc_I4, index);
            ctorBuilder.EmitLoadTypeNameString(
                assemblyContext.Imports,
                originalMethod,
                originalMethod.Parameters[index].ParameterType,
                method.NewMethod.Parameters[index].ParameterType);
            ctorBuilder.Add(OpCodes.Stelem_Ref);
        }

        ctorBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_GetIl2CppMethodExact.Value);
    }
