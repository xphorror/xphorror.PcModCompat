        ...
        foreach (var assemblyContext in context.Assemblies)
        {
            foreach (var typeContext in assemblyContext.Types)
            {
                foreach (var methodRewriteContext in typeContext.Methods)
                {
                    ...
                    var methodIdentity =
                        $"{originalMethod.DeclaringType?.DeclaringModule?.Name}:" +
                    ...
                    if (typeContext.ComputedTypeSpecifics != TypeRewriteContext.TypeSpecifics.BlittableStruct)
                    {
                        if (originalMethod.IsConstructor)
                        {
                            ...
                            bodyBuilder.Add(OpCodes.Ldstr, methodIdentity + " instance class");
                            bodyBuilder.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppClass.Value);
                            bodyBuilder.Add(OpCodes.Call, imports.IL2CPP_il2cpp_object_new.Value);
                            bodyBuilder.Add(OpCodes.Ldstr, methodIdentity + " instance allocation");
                            bodyBuilder.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppObject.Value);
                        ...
                        ((originalMethod.IsVirtual && !originalMethod.DeclaringType.IsValueType()) || originalMethod.IsAbstract))
                    {
                        ...
                        bodyBuilder.Add(OpCodes.Ldstr, methodIdentity + " virtual definition");
                        bodyBuilder.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppMethod.Value);
                    ...
                    bodyBuilder.Add(OpCodes.Ldstr, methodIdentity + " invoke target");
                    bodyBuilder.Add(OpCodes.Call, imports.IL2CPP_RequireIl2CppMethod.Value);
