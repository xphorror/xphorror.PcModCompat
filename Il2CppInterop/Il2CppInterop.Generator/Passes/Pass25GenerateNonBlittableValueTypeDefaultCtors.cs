    ...
    public static void DoPass(RewriteGlobalContext context)
    {
        foreach (var assemblyContext in context.Assemblies)
            foreach (var typeContext in assemblyContext.Types)
            {
                ...
                var typeIdentity =
                    $"{typeContext.OriginalType.DeclaringModule?.Name}:{typeContext.OriginalType.FullName}";
                bodyBuilder.Add(OpCodes.Ldarg_0);
                bodyBuilder.Add(OpCodes.Ldsfld, typeContext.ClassPointerFieldRef);
                bodyBuilder.Add(OpCodes.Ldstr, typeIdentity + " default value class");
                bodyBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_RequireIl2CppClass.Value);
                bodyBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_il2cpp_object_new.Value);
                bodyBuilder.Add(OpCodes.Ldstr, typeIdentity + " default value allocation");
                bodyBuilder.Add(OpCodes.Call, assemblyContext.Imports.IL2CPP_RequireIl2CppObject.Value);
