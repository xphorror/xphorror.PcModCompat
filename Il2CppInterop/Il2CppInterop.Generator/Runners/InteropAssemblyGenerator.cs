...
internal class InteropAssemblyGeneratorRunner : IRunner
{
    public void Run(GeneratorOptions options)
    {
        ...
        else
        {
            Logger.Instance.LogInformation(
                "Skipping generated-corlib implicit conversions and awaiter helpers for dependency-closed proxy output.");
        }
        ...
        if (!options.RuntimeMetadataOnly)
        {
        ...
        {
            DeleteAddressArtifact(options.OutputDir!, MethodXrefScanCache.FileName);
        }
        ...
        {
            DeleteAddressArtifact(options.OutputDir!, MethodAddressToTokenMapBase<Assembly, MethodBase>.FileName);
        }
    ...
    private static void DeleteAddressArtifact(string outputDir, string fileName)
    {
