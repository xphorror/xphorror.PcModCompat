...
public class Traverse<T>
{
    ...
    // Upstream's private parameterless constructor leaves the backing field null and is unreachable
    // from outside; the explicit null! only silences the nullable warning, the state is identical.
...
// ABI mirror of Harmony 2.4 HarmonyLib/Tools/Traverse.cs.
//
// Fluent private-member reflection. Everything here goes through FieldInfo/PropertyInfo GetValue and
// SetValue plus MethodBase.Invoke - no IL is read or emitted, and upstream has no FieldRefAccess
// dependency here - so the whole type reproduces upstream verbatim. Only nullable annotations differ.
/// <summary>A reflection helper to read and write private elements</summary>
///
