...
// ABI mirror of Harmony 2.4 HarmonyLib/Tools/Code.cs.
//
// A MOD that writes `using static HarmonyLib.Code;` gets ~220 named CodeMatch subclasses, one per
// opcode, plus an indexer to attach an operand and a name. Every one of them is a pattern object, not
// an emitter, so the whole file mirrors upstream mechanically - generated from the upstream source
// with only the nullable annotations added, so a missing entry cannot creep in by hand-copying.
