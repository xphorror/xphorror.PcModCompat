...
// ABI mirror of Harmony 2.4 HarmonyLib/Internal/AccessCache.cs.
//
// The member-lookup cache behind Traverse. Pure reflection with a per-dictionary lock, no IL read or
// emitted, so this reproduces upstream verbatim. Note that upstream caches negative results too - a
// name that does not resolve is stored as null and never re-fetched - and that is kept here.
internal class AccessCache
{
    ...
    {
        var value = Get(declaredMethods, type, name, arguments, () => (MethodBase?)type.GetMethod(name, declaredOnlyBindingFlags[memberType], null, arguments, null));
        if (value is null && declaredOnly is false)
            value = Get(inheritedMethods, type, name, arguments, () => (MethodBase?)AccessTools.Method(type, name, arguments));
        return value;
    }
}
